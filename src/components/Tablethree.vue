    ...
    <div class="py-2 inline-block min-w-full pr-5">
      <div class="overflow-hidden rounded-lg">
            ...
            <div class="flex">
               <button class="flex border-solid border border-gray-300 px-3 py-2 mx-6 rounded">
                ...
                <div class="flex ml-8">
                    ...
                    <input v-model="searchUnpaidElems" @input="filteredUnpaidList" type="text" placeholder="Search Users by Name, Email or Date" class="text-sm w-[340px] px-8 py-1 bg-gray-100 outline-blue-200 font-semibold">
                </div>
            </div>
        ...
        <table class="min-w-full ">
          ...
          <tbody>
            <tr class="bg-white border-b" v-for="user in filteredUnpaidUsers" :key="user.id">
...
const searchUnpaidElems = ref("");

const unpaidStore = useUserData();
const {unpaidUserAmounts,getPaymentStatusColor, getPaymentStatusBackgroundColor}  = storeToRefs(unpaidStore);

onMounted(() =>{
console.log("userAmounts>>", unpaidUserAmounts.value);
})

const filteredUnpaidUsers = computed(() => {
  return unpaidUserAmounts.value.filter(item =>
    item.name.toLowerCase().includes(searchUnpaidElems.value.toLowerCase()) || item.email.toLowerCase().includes(searchUnpaidElems.value.toLowerCase())
   );
});

const filteredUnpaidList = () => {
